...
[Dynamic Salience Map v1]

영상 분석 시 고정 도메인 구조를 강제하지 마라.
영상마다 "빼면 글이 망가지는 핵심 재료" 가 다르다. 먼저 그것을 찾아서
preservation_targets 에 박아라. 특정 도메인 (예측시장 / 의료 / 정책 /
기술 등) 의 사례명을 모든 영상에 강제하지 마라.

7 카테고리 (영상마다 비중이 다름 — 모두 항상 채울 필요 없음, 영상에 있는
것만):

1. Core Entities
- 영상의 핵심 인물 / 기업 / 플랫폼 / 기관 / 국가 / 프로젝트
- 비교 대상이 2 개 이상이면 모두 보존 (한쪽만 남기지 마라)

2. Core Numbers
- 영상의 핵심 숫자 — 금액, 수익률, 비율, 기간, 거래량, 사용자 수,
  기업가치, 성장률, 사망자, 임상 결과, 점유율 등
- 숫자가 글의 몰입과 신뢰를 만드는 경우 preservation_targets 에 넣어라

3. Core Events
- 영상에서 가장 중요한 사건 — 대선, 전쟁, 공습, 규제 발표, 기업 성장,
  기술 변화, 임상 발표, 정책 시행 등
...
- 영상의 핵심 대비. 예:
  · 예측 도구 vs 도박장
  · AI 생산성 vs 블랙박스 책임
  · 팬데믹 가능성 낮음 vs 대응 체계 리스크
  · 성장하는 플랫폼 vs 손해 보는 개인

5. Core Mechanism
- 이 영상이 설명하는 작동 구조. 예:
  · 누가 돈을 버는가
  · 누가 손해 보는가
  · 어떤 기술/제도/시장 구조가 작동하는가
  · 왜 이런 일이 생기는가

6. Core Outcome
- 영상이 최종적으로 말하려는 결론. conclusion_claim 에 별도 추출.

7. Must-Keep Items
- 이 항목이 빠지면 글이 틀어지는 핵심 재료. preservation_targets 에 명시.

도메인 적응 (참고만 — 강제 X):
- 경제 영상 → 돈 흐름 / 누가 버는지 가 중요할 수 있음
- 의료 영상 → 위험도 / 대응 체계 / 환자 outcome 이 중요할 수 있음
- 기술 영상 → 작동 원리 / 한계 / 비용 구조 가 중요할 수 있음
- 정책 영상 → 이해관계자 / 규제 변화 / 시행 시점 이 중요할 수 있음
- 인물/다큐 → 장면 / 결정 / 맥락 이 중요할 수 있음
영상마다 핵심 재료가 다르다. 위 도메인 패턴은 강제가 아니라 참고용.

preservation_targets 작성 기준:
- 반드시 포함해야 할 기업 / 인물 / 기관
- 반드시 포함해야 할 숫자
- 반드시 포함해야 할 사건
- 반드시 포함해야 할 대비
