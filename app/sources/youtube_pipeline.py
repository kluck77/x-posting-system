...
[Salience-Locked Economic Spine v1]

경제/금융/코인/AI 산업/플랫폼/예측시장/투자 관련 영상에서는 아래 7 항목을
반드시 추출한다. 본문 작성자가 중요도를 잃지 않도록 preservation_targets
필드에 반드시 반영한다. 영상에 없는 수치/기업명/주장 강도를 새로 만들지
마라. 불확실한 수치는 UNCERTAIN 으로 표시하라.

1. Key Entities
- 핵심 기업 / 플랫폼 / 프로젝트 / 인물 / 기관을 모두 추출
- 영상에서 비교 대상 (예: Polymarket vs Kalshi) 이 나오면 둘 다 보존
- 한쪽만 남기지 마라

2. Growth Metrics
- 기업가치 / 거래량 / 이용자 수 / 시장점유율 / 성장률
- 투자 유치액 / 매출 / 수수료 / 거래 규모 / 기간 비교
- 영상에 나온 숫자는 모두 보존
...
4. Retail Outcome
- 개인 이용자 / 개미 / 일반 참여자의 평균 수익률
- 손실 비율 / 중간 수익률
- 소수 승자와 다수 패자 구조
- 영상에 데이터가 있으면 반드시 보존

5. Winner / Loser Map
- 승자: 플랫폼, 정보 우위자, 내부자, 마켓메이커, 유동성 공급자
- 패자: 일반 이용자, 늦게 들어온 사람, 정보 부족한 사람
- 영상에서 말한 범위 안에서만 정리

6. Controversy / Regulation
- 도박성 / 내부자 거래 / 정치·전쟁·사망·재난 베팅
- 규제기관 반응 / 윤리 논란 / 플랫폼 방어 논리
- 보존하되 경제 구조보다 먼저 나오지 않게 한다 (배치 우선순위)

7. Must-Keep Spine — preservation_targets 형식
preservation_targets 안에 핵심 뼈대를 명시적으로 박는다. 예:
- "반드시 Polymarket 과 Kalshi 를 함께 설명할 것"
- "두 플랫폼의 성장 수치를 누락하지 말 것"
- "개인 이용자의 평균 수익률 / 손실 구조를 누락하지 말 것"
- "누가 돈을 버는지 설명할 것"
- "윤리 논란은 경제 구조 설명 뒤에 배치할 것"
"""
