...
def _build_mode_slot_instruction(mode: str) -> str:
    ...
    return (
        ...
        "  슬롯2 = 아직 확인 안 된 것 (어떤 공백/출처 미검증이 있는가)\n"
        "        ※ 이 슬롯은 UI 상 '왜 뉴스 이상이냐' 자리에 들어가지만,\n"
        "          VERIFY 에서는 '구조적 의미' 서술 금지.\n"
        "          오직 '어떤 공백이 있는가' 한 줄로 약화하라.\n"
        "  슬롯3 = 무엇이 확인되면 진짜인지 (검증 신호)\n"
        "금지(강): 정치적 계산 / 숨은 의도 / 본심 / 노림수 / 전략적 카드 / "
        "체제 양보 / 질서 재편 / 국제질서 재편 / 구조적 변화 단정 / "
        "장기 지정학 해석 / 제3국을 통한 실질적 대화 채널 / 외교 채널 "
        "복원 / 협상 의제 연동 / 긴장 수위 상승 / 구조적 의미 / "
        "다음 국면을 결정 / 진짜 신호.\n"
...
def _build_mode_finalize_instruction(mode: str) -> str:
    ...
    return (
        "\n━━━ 지시 (MODE: VERIFY) ━━━\n"
        "미확인/단독/루머성 기사. **최대 3문장**만으로 확인 축만 잡는다.\n"
        "장기 해석 · 의도 추정 · 구조 재편 · 외교 시나리오 확장 전면 금지.\n"
        "\n"
        "문장 1 (첫 줄): 1문장 1주장. '지금 나온 말 한 줄'로만 고정.\n"
        "  금지(첫 줄):\n"
        ...
        "\n"
        "본문 전역 금지(강):\n"
        "  ✗ '제3국을 통한 실질적 대화 채널' '외교 채널 복원'\n"
        "  ✗ '협상 의제 연동' '긴장 수위 상승' '구조적 의미'\n"
        "  ✗ '실효성 여부가 판가름' '다음 국면을 결정' '진짜 신호'\n"
        "  ✗ '정치적 계산' '숨은 의도' '본심' '노림수' '전략적 카드'\n"
        "  ✗ '체제 양보' '질서 재편' '국제질서 재편' '구조적 변화'\n"
        "\n"
        "본문 전역 허용:\n"
        "  ✓ '~측이 ~라고 밝혔다'\n"
        "  ✓ '~은 아직 확인되지 않았다'\n"
        ...
        "\n"
        "핵심 원칙: VERIFY 에서는 '무슨 일이 벌어질 수 있다' 가 아니라 "
