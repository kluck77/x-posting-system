...
def _format_handoff_youtube_slim(final_body: str, *, body_max: int = 3500) -> str:
    ...
    instruction = (
        ...
        "한다. 긴 해설보다 첫 줄, 핵심 숫자, 짧은 항목, 구역 구분, 가독성을 "
        "우선하라. 재료에 따라 CURRENT_ODDS_COMPARE, WHY_MARKET_HOLDS, "
        "DATA_LEDGER, SHORT_SIGNAL 중 자연스러운 글 모양을 선택하거나 "
        ...
        "- 중복 제거\n"
        "- 문장 리듬 개선\n"
        "- 첫 줄/둘째 줄 강화 (둘째 줄에는 핵심 숫자 / 상태)\n"
        "- 마지막 문장은 짧고 단단한 판단 1 줄\n"
        "- 내부 분석 라벨 제거: '⚠️ 진짜 쟁점' / '📌 지금 봐야 할 포인트' "
        "/ '🔥 왜 세게 써야 하는가' / '💎 반드시 살릴 포인트' / "
        "'🎯 살릴 가치'\n"
        "- 보고서 말투 / 뉴스 해설형 장문 / 유튜브 리뷰체 제거\n"
        "- '주목된다' / '의미가 있다' / '시사점을 준다' / "
        "'여러분의 생각은?' / '댓글로 남겨주세요' 같은 결말 제거\n"
        "- 편집 결과만 출력"
    )
    ...
    *,
    ...
    if _src == "news_link":
        section_house_format = [
            "## House Format 편집 지시",
            "이 글은 뉴스 요약이나 영상 리뷰가 아니라 모바일 스캔형 X "
            "포스트여야 한다. 긴 해설보다 첫 줄, 핵심 숫자, 짧은 항목, "
            "구역 구분, 가독성을 우선하라. 재료에 따라 "
...
# ─── 외부 Grok 편집용 마스터 프롬프트 (사용자가 외부 Grok 에 복붙) ─────────
# 시스템 내부 handoff (format_handoff / _format_handoff_youtube_slim) 와
# 동일 방향. 운영자가 외부 Grok UI 에 직접 붙여 넣을 때 사용. 특정 기업/
# 정치인/코인/국가명 prompt 박힘 0. 코드 경로(app/services)에 두는 이유:
# docs 디렉터리가 아직 없고, 테스트가 import 로 검증 가능해야 함.
SCAN_FIRST_GROK_EDITOR_PROMPT_V1 = """[SCAN_FIRST_GROK_EDITOR_PROMPT_V1]

너는 X 포스트 전문 편집자다.

목표:
사용자가 준 초안 또는 handoff 를 뉴스 요약, 유튜브 리뷰, 보고서 문체가
아니라 모바일에서 빠르게 읽히는 X 포스트로 편집한다.
...
- 첫 화면에 핵심 숫자 / 판세 / 상태 / 질문이 보여야 한다.
- 문단보다 항목 구조를 우선한다.
- 한 문단은 짧게 유지한다.
- 숫자는 문장 속에 묻지 말고 눈에 띄게 배치한다.
- 이미지가 있는 글은 본문을 더 짧고 명확하게 만든다.

[선택 가능한 글 모양 — 4 후보, 고정 템플릿 아님]

재료에 맞는 형식을 자연스럽게 선택하거나 혼합한다.

1. CURRENT_ODDS_COMPARE
- 확률 / 판세 / 후보 / 선택지 / 진영 / 비교가 핵심일 때 사용
- 첫 줄: 현재 판세 또는 시장 기대치
- 둘째 줄: 핵심 숫자 비교
- 본문: 양쪽 선택지의 핵심 항목 비교
- 마지막: 이 비교가 보여주는 구조적 의미

2. WHY_MARKET_HOLDS
- 시장이 왜 특정 포지션을 유지하는지 설명할 때 사용
- 첫 줄: 강한 질문 또는 이상 현상
- 둘째 줄: 현재 핵심 숫자 / 상태
- 본문: N 가지 이유
...
3. DATA_LEDGER
- 수익률 / 순위 / 보유량 / 거래량 / ETF / 기업 리스트 / 국가 비교 /
  섹터 비교에 사용
- 제목 / 기준일·기간 / 카테고리 / 항목명 / 숫자 / (이미지·차트)
- 긴 해설보다 숫자 스캔을 우선

4. SHORT_SIGNAL
- 강한 숫자 하나 / 강한 이미지 하나 / 짧은 헤드라인 하나로 충분할 때 사용
- 1~4 줄 우선
- 본문에서 다 설명하지 않는다
- 정보 공백을 남겨 클릭을 유도한다
- 해석은 필요하면 첫 댓글로 넘긴다
...
- 색상 이모지, 돈 / 시장 / 국가 구분 이모지는 허용한다.
- 아래 같은 내부 분석 라벨은 제거한다:
  · 진짜 쟁점
  · 지금 봐야 할 포인트
  · 왜 이 글을 세게 써야 하는가
  · 반드시 살릴 포인트
