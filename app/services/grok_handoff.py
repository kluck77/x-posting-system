...
def _format_handoff_youtube_slim(final_body: str, *, body_max: int = 3500) -> str:
    ...
    prohibitions = (
        ...
        "- 원문 주장 강도를 높이지 말 것\n"
        "- \"가능성\"을 \"확정\"으로 바꾸지 말 것\n"
        "- \"영향을 줄 수 있다\"를 \"무너뜨린다\"로 바꾸지 말 것\n"
        "- 초안에 A 와 B 가 모두 있어도, 초안이 직접 연결하지 않았다면 "
        "\"A 때문에 B\" 로 쓰지 말 것\n"
        "- 의료·정책·금융·법률·전쟁 주제에서는 공포·확정·붕괴 표현을 "
        "새로 추가하지 말 것\n"
        "- 출처 없는 예측은 전망/우려/가능성 수준으로 낮출 것\n"
        "- 문장은 다듬되 주장은 확장하지 말 것\n"
        "- 문장 리듬은 다듬되, 주장은 확장하지 말 것\n"
        "- 첫 문장은 강화하되, 원문에 없는 사실을 넣지 말 것\n"
