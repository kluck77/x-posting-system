...
_INTERPRETATION_SIGNALS_KO = re.compile(
    ...
    r"사실상|구조적|구조는|구조|본질|본질적|진짜 이유|진짜 원인|원인은|"
    r"주목할|주목해야|보여준다|보여주는|드러난|드러내|"
    r"시사한다|시사하는|시사점|결국|결과적으로|"
    r"메커니즘|전이|파급|파급효과|완충|단계적|"
    r"변수|요인|경로|연쇄|역학|함의|전환점|분기점|프레임|"
    # 정치·외교·군사 분석 어휘
    r"압박|카드|포석|레버리지|노림수|전략적|양면|"
    r"계산|의존도|셈법|교섭|명분|실익|딜레마"
)
...
_MARKETABILITY_SIGNALS_KO = re.compile(
    ...
    r"미국|중국|일본|유럽|대만|인도|"
    r"물가|인플레이션|석유|에너지|정유|GDP|성장률|"
    # 지정학·안보 (해외 독자 관련성 높음)
    r"봉쇄|제재|동맹|안보|방위비|호르무즈|중동|NATO|나토"
)
...
_REPEAT_SIGNALS_KO = re.compile(
    ...
    r"모니터링|확인해야|살펴봐야|점검|지표를"
)

# "충격"은 경제 분석 표준 용어(유가 충격, 외부 충격 등)이므로 제외.
# 영어 패턴도 "shocking"(클릭베이트 형용사)만 잡고 "shock"(경제 용어)는 안 잡음.
...
_PILLAR_SIGNALS_KO = re.compile(
    ...
    r"유가|원유|에너지|석유|정유|물가|인플레이션|원자재|"
    # 군사·안보 필러
    r"군사|동맹|안보|방위비|주한미군|주둔군|병력|봉쇄|제재"
)
