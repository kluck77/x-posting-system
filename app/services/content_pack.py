...
문장 3 (검증 포인트):
  - 구체적 다음 포인트로 끝낸다
  - 조건형/대비형/질문형만 허용
  - 예: "진짜 포인트는 여야가 지역별로 다른 처방을 내놓느냐다."

2~3문장이 기본. 4문장까지 허용. 그 이상 금지.
...
□ 2. 마지막 문장이 조건형/대비형/질문형인가? (전망문/훈계문이면 실패)
...
async def generate_final_post(
    ...
    user_prompt += (
        "\n━━━ 지시 ━━━\n"
        "위 해석 슬롯과 팩트만으로 2~3문장 게시글을 조립하라.\n"
        "기사 요약 금지. 슬롯의 해석 방향만 쓴다.\n\n"
        "문장 1: 변화/갈등/차이 — 긴장점을 드러내라\n"
        "문장 2: 팩트 1개 — 근거를 짧게 붙여라\n"
        "문장 3: 다음 검증 포인트 — 조건형/대비형으로 끝내라\n\n"
        "- opener는 참고만. 그대로 복붙하지 마라.\n"
        "- 마지막 문장: 조건형/대비형/질문형만 허용.\n"
        "  ✗ 심각하다/중요하다/반영한다/불가피하다/변수다/관건이다/확인이 필요하다\n"
