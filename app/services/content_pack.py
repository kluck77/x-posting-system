...
_FINALIZE_PROMPT_KO = """너는 한국 이슈 해설형 X 계정의 "슬롯 조립 담당"이다.
...
문장 1 — 변화 또는 긴장점:
  - 지금 바뀐 것 / 갈라지는 지점 / 독자가 바로 이해해야 할 충돌
  - 기사 요약 금지. "~가 발표됐다" "~것으로 전해졌다" 금지.
  - 예: "서울은 집값, 인천은 경기다. 같은 선거인데 민심의 우선순위가 갈린다."
...
문장 3 — 해석 또는 구조적 의미 (선택):
  - 기사보다 한 단계 위의 해석. 단, 추상 명분형 해설 금지.
  - 이 문장이 없어도 되지만, 있으면 해석 밀도가 올라간다.

문장 마지막 — 외부 검증 신호 (CRITICAL):
  - 반드시 "확인할 대상"으로 끝낸다. "생각"으로 끝내지 마라.
  - 3종만 허용:
    ✓ SIGNAL_CHECK: 어떤 외부 신호가 나오면 진짜/가짜가 판별되는가
    ✓ THRESHOLD_CHECK: 어느 수치/조치/후속 문서가 나오면 의미가 달라지는가
    ✓ IMPLEMENTATION_CHECK: 실제 집행 여부를 확인할 구체 단계가 무엇인가
  - 좋은 예:
    ✓ "봉쇄 발표가 해군 이동이나 추가 제재로 이어지느냐가 진짜 분기점이다."
    ✓ "브렌트 상승분이 며칠 유지되는지 보면 단기 쇼크인지 추세인지 갈린다."
    ✓ "시행령/가이드라인/배제 명단 공개 여부를 보면 선언인지 정책인지 알 수 있다."
  - 절대 금지:
...
□ 1. 첫 문장이 긴장점(갈등/차이)을 드러내는가? (기사 요약이면 실패)
□ 2. 마지막 문장이 외부 검증 신호인가? (뭘 보면 판별되는지 구체적으로 나와야 통과. "관건이다/변수다/확인 필요"면 실패)
□ 3. 기사를 다시 설명하는 문장이 있는가? (있으면 삭제)
□ 4. cautions와 충돌하는 표현이 없는가?
□ 5. certainty 미확인인데 동기 추정("정치적 계산/숨은 의도")이 중심 서사인가? (있으면 톤 낮추기)
...
async def _gemini_generate_thesis_cards(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            logger.info(
                f"[API-COST] gemini gemini-2.0-flash "
            ...
            try:
                from app.services.api_cost_tracker import record_usage
                record_usage(
                    "gemini", "gemini-2.0-flash", "GeminiThesisCards",
...
async def generate_final_post(
    ...
    user_prompt += (
        "\n━━━ 지시 ━━━\n"
        "위 해석 슬롯과 팩트만으로 2~4문장 고품질 게시글을 조립하라.\n"
        "기사 요약 금지. 슬롯의 해석 방향만 쓴다.\n"
        f"{_speculation_guard}\n"
        "문장 1: 변화/갈등/차이 — 긴장점을 드러내라\n"
        "문장 2: 팩트 근거 — 핵심 숫자/조치를 짧게 붙여라\n"
        "(선택) 문장 3: 구조적 의미 — 기사 한 단계 위의 해석\n"
        "마지막 문장: 외부 검증 신호 — 뭘 보면 판별되는지 구체적으로 써라\n"
        "  ✓ '브렌트 상승분이 며칠 유지되는지 보면 단기 쇼크인지 추세인지 갈린다'\n"
        "  ✗ '관건이다' '변수다' '확인이 필요하다' '지켜봐야 한다'\n\n"
...
4. fail_tags 활용:
   ...
   - DEAD_ENDING → 마지막 문장을 외부 검증 신호로 교체 (뭘 보면 판별되는지).
   - SPECULATIVE_MOTIVE → 동기 추정 톤 낮추기. certainty 미확인이면 특히 약화.
...
문장 1 — 변화 또는 긴장점: 독자가 왜 이걸 봐야 하는가. 기사 요약 금지.
문장 2 — 팩트 근거: 확인된 사실 1~2개 + 해석 축 연결. 나열 금지.
(선택) 문장 3 — 구조적 의미: 기사 한 단계 위의 해석.
마지막 문장 — 외부 검증 신호: 뭘 보면 판별되는지 구체적으로. 전망문·훈계문 금지.

2~4문장. 해석 밀도 우선. 칼럼체 금지.
...
5. 금지 마감 패턴 → "외부 검증 신호"로 교체 (뭘 보면 판별되는지 구체적으로)
...
아래 패턴이 마지막 문장에 있으면 반드시 "외부 검증 신호"로 교체:
...
교체 = "외부 검증 신호" (뭘 보면 판별되는지 구체적으로):
  ✓ "관세 부과 시점이 다음 분기 실적을 가른다"
  ✓ "해군 이동이나 추가 제재가 뒤따르느냐가 분기점이다"
  ✓ "브렌트 상승분이 며칠 유지되는지 보면 단기 쇼크인지 추세인지 갈린다"
  ✓ "실제 시행령이 나와야 선언인지 정책인지 윤곽이 잡힌다"
...
3. rewrite_scope
   ...
   주의: DEAD_ENDING만 있어도 최소 REWRITE_OPENER_ONLY 이상. 마지막 문장 교정 필요.
   주의: SPECULATIVE_MOTIVE + certainty 미확인 → 강한 감점 (score -2 수준)
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            try:
                from app.services.api_cost_tracker import record_usage
                record_usage(
                    "gemini", "gemini-2.0-flash", "GeminiOpinionCard",
                    usage.get("promptTokenCount", 0),
                    usage.get("candidatesTokenCount", 0),
                )
            except Exception:
