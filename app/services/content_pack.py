...
문장 마지막 — 판별 신호 (CRITICAL):
  ...
  - 좋은 예:
    ...
    ✓ "브렌트 상승분이 3영업일 유지되면 단기 쇼크가 아니라 추세다."
...
□ 2. 판단 좌표가 문장에 녹아 있는가? (독자가 새 해석 기준을 얻는가?)
□ 3. 마지막 문장이 판별 신호인가? ("무엇이 나오면/안 나오면" 구체적으로 나와야 통과. "관건이다/변수다/확인 필요"면 실패)
□ 4. 기사를 다시 설명하는 문장이 있는가? (있으면 삭제)
□ 5. cautions와 충돌하는 표현이 없는가?
□ 6. certainty 미확인인데 동기 추정("정치적 계산/숨은 의도")이 중심 서사인가? (있으면 톤 낮추기)
...
async def generate_final_post(
    ...
    user_prompt += (
        ...
        "문장 3: 판단 좌표 — 독자가 새로 갖게 되는 해석 기준을 녹여라\n"
        "마지막 문장: 판별 신호 — '무엇이 나오면/안 나오면' 확인 가능한 구체적 검증 포인트로 끝내라\n"
        "  ✓ '후속 시행령이 6월 전에 나오면 정책, 안 나오면 선언에 그침'\n"
        "  ✗ '관건이다' '변수다' '확인이 필요하다' '지켜봐야 한다'\n\n"
...
마지막 문장 — 판별 신호: "무엇이 나오면/안 나오면" 구체적 검증 포인트. 전망문·훈계문 금지.
