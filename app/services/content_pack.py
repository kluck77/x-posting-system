...
■ short_version 규칙
═══════════════════════════════════════════
- 메인 포스트를 단순 축약하지 마라. 별도의 훅형 압축문을 써라.
- 200자 이내. 본문 없이 훅 한 줄로 완결.

✅ 좋은 예: "한은 금리 동결 5연속 — 시장은 인하 베팅 시작했다"
❌ 나쁜 예: "한국은행이 금리를 동결했습니다. 이는 5번째 동결입니다." ← 메인 축약일 뿐

═══════════════════════════════════════════
■ why_it_matters 규칙
═══════════════════════════════════════════
한 문장으로: ① 영향받는 대상, ② 해외 독자가 관심 가질 이유, ③ 확인할 수 있는 지표를 포함하라.
추상적 중요성("경제에 중요한 의미") 금지. 구체적 파급 경로를 써라.

✅ 좋은 예:
- "미국 금리 인하 기대와 반대로 가면 원화와 외국인 자금 흐름이 먼저 흔들린다"
- "반도체 재고 사이클 꺾이면 KOSPI 외국인 순매수 추세가 함께 꺾인다"
- "중국 관광객 회복 속도가 면세점·항공주 밸류에이션의 키다"
- "전세 시장 불안이 가계부채 건전성 지표에 직접 연결된다"
- "조선업 수주 잔고가 사상 최고인데 인력난이 인도 일정을 위협한다"

❌ 나쁜 예 (절대 이렇게 쓰지 마라):
- "이는 경제에 중요한 의미를 가진다" ← 아무 정보 없음
...
✅ 좋은 예:
- "시장은 금리보다 환율을 먼저 보고 있는 것 같다"
- "올해 들어 외국인 채권 매수가 3배 늘었는데, 금리 동결 신호로 읽는 거 아닌가"
- "조선소 현장 가보면 인력난이 진짜 심각하다. 하청 단가가 작년 대비 40% 올랐다"
- "서울 아파트 실거래가 지수 보면 강남 빼고는 아직 하락 추세인데?"
...
- 아래 3가지 유형 중 택 1 (포스트당):
  ① 반론형: "근데 진짜 문제는 ~"
  ② 시장영향형: "이게 ~에 미치는 영향은 ~"
  ③ 해외설명형: "한국 사정 모르면 놓치는 맥락: ~"
- 각 220자 이내.
