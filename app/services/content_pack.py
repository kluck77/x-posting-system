...
8. 데이터 없는 단정 금지: 경제·부동산·금리·환율·주식 주제에서 아래 3개 중 최소 2개가 없으면 강한 단정("상승했다" "회복됐다" "반등했다")을 쓰지 마라.
   ① 지역/대상 (어디? 무엇이?)
   ② 기간/비교 시점 (언제 대비?)
   ③ 수치/변화폭 (얼마나?)
...
■ 최종 QA 체크리스트 (STEP 3)
═══════════════════════════════════════════
JSON 출력 전에 아래 8개를 내부적으로 점검하라. 3개 이상 실패하면 전체 재작성.

□ 1. 제목 재진술 아닌가? — 훅이 기사 제목 복사면 실패
□ 2. 데이터 충족? — 경제·부동산 글에서 지역/기간/수치 중 2개 이상 있는가
□ 3. 메인 축 분리? — A/B/C가 서로 다른 핵심축인가
□ 4. 댓글 답글형? — 각 1~2문장이고 대화체인가
