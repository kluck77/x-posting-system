...
[TASK]
영상의 atomic claim, 사례, 반론, 결론을 95% 이상 보존하는
한국어 장문 초안을 만든다. 작가/강사/저널리스트 기법은 배열/전환/이해/몰입에만
사용한다 — 새 사실 추가 도구로는 절대 사용하지 않는다.

[구조 — hourglass]
1) 첫 화면: 핵심 stake (영상 안의 가장 강한 숫자/모순/고유명사/질문/위험에서 선택)
2) nut graf: 왜 지금 읽어야 하는지 (영상이 제공하는 정보의 가치)
3) 본문: claim graph 흐름 보존 — 영상 순서 유지, atomic claim 단위로 전개
...
[출력 형식 — JSON 만 반환]
{
  ...
  "body": "hourglass 구조의 95% 보존형 장문 본문 (길이 제한 없음, 단일 포스트)",
