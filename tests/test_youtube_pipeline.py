...
# ─── House X Post Shape v1 — 하우스 스타일 (고정 템플릿 아님) ────────
class TestHouseXPostShapeV1:
    """[A] OpenAI YouTube prompt 에 House X Post Shape v1 블록.
    [B] 핵심 기준 5 항목 (첫 줄 / 둘째 줄 / N가지 / 어려운 개념 / 마지막).
    [C] Grok handoff 에 하우스 스타일 편집 지시 1 줄.
    [D] schema/interface/모델 보존."""

    # ── A. OpenAI prompt 에 House Shape ──────────────────────────────
    def test_openai_youtube_has_house_shape_block(self):
        from app.providers.openai_provider import (
            YOUTUBE_DIGEST_SYSTEM_PROMPT,
            YOUTUBE_HOUSE_X_POST_SHAPE_V1,
        )
        assert "House X Post Shape v1" in YOUTUBE_HOUSE_X_POST_SHAPE_V1
        assert "House X Post Shape v1" in YOUTUBE_DIGEST_SYSTEM_PROMPT

    # ── B. 핵심 기준 5 항목 ─────────────────────────────────────────
    def test_openai_youtube_has_house_shape_principles(self):
        from app.providers.openai_provider import YOUTUBE_DIGEST_SYSTEM_PROMPT
        for kw in (
            "주제 설명으로 시작하지 말 것",
            "독자가 계속 읽을 이유를 준다",
    ...
    def test_openai_youtube_house_shape_forbids_review_styles(self):
        from app.providers.openai_provider import YOUTUBE_DIGEST_SYSTEM_PROMPT
        # 금지 항목 — 뉴스 요약체 / 유튜브 리뷰체 / 보고서 말투
        for forbidden in (
            "뉴스 요약체",
            "유튜브 리뷰체",
            "보고서 말투",
    ...
    def test_openai_youtube_keeps_prior_layers(self):
        # 직전 PR 들의 핵심 구조 (Salience-First / Truth Integrity / 5 모드 /
        # Framework Lock) 가 그대로 살아있는지
        from app.providers.openai_provider import YOUTUBE_DIGEST_SYSTEM_PROMPT
        for kw in (
            "Salience-First Draft Planner",
            "Truth Integrity",
    ...
    # ── C. Grok handoff 에 하우스 스타일 편집 지시 ──────────────────
    ...
    def test_non_youtube_handoff_no_house_style(self):
        # non-YouTube lane 에 하우스 스타일 1 줄이 새지 말 것
        from app.services.grok_handoff import format_handoff
        sp = {"confirmed_facts": ["사실 1"], "evidence_pack": [],
              "concept_translation": ""}
        ap = {"winner_angle": {"angle": "앵글"}, "core_tension": "긴장",
              "frame_type": "x", "story_spine": []}
        out = format_handoff(sp, ap, "본문", source_type="news_link")
        assert "독립적인 X 글" not in out
        assert "뉴스 요약이나 영상 리뷰가 아니라" not in out
        assert "House X Post Shape" not in out
    ...
    def test_general_ko_prompt_no_house_shape(self):
        ...
        for kw in (
            ...
            assert kw not in SYSTEM_PROMPT_KO, (
                f"일반 KO prompt 에 YouTube 전용 House Shape '{kw}' 새면 안 됨"
            )
